...
class Cos_Test {
		...
		class RadianBoundryTest{
			...
			class PostiveRadianBoundryTest{
				...
				public void TestWithZero() {
					...
					Assert.assertEquals(1.0,Cos_FinalVersion.calculate(radian),0.000001);
				}
				
				@Test
				public void TestWithHalfPI() {
					IAngleMeasurement radian = new RadianMeasurement(Maths.PI/2);
					Assert.assertEquals(0.0,Cos_FinalVersion.calculate(radian),0.000001);
				}
				...
				@Test
				public void TestWithTwoThirdPI() {
				...
				public void JustAboveTestWithZero() {
					...
					Assert.assertEquals(0.995004,Cos_FinalVersion.calculate(radian),0.000001);
				}
				
				@Test
				public void JustAboveTestWithHalfPI() {
					IAngleMeasurement radian = new RadianMeasurement(Maths.PI/2+0.1);
					Assert.assertEquals(-0.099833,Cos_FinalVersion.calculate(radian),0.000001);
				}
			...
			class NegativeRadianBoundryTest{
				...
				@Test
				public void radNegativeBoundryTestWithHalfPI() {
					IAngleMeasurement radian = new RadianMeasurement((-1)*Maths.PI/2);
					Assert.assertEquals(0.0,Cos_FinalVersion.calculate(radian),0.000001);
				}
				
				@Test
				public void radNegativeBoundryTestWithPI() {
					IAngleMeasurement radian = new RadianMeasurement((-1)*Maths.PI);
					Assert.assertEquals(-1.0,Cos_FinalVersion.calculate(radian),0.000001);
				}
				
				@Test
				public void radNegativeBoundryTestWithTwoThirdPI() {
					IAngleMeasurement radian = new RadianMeasurement((-1)*Maths.PI*3/2);
					Assert.assertEquals(0.0,Cos_FinalVersion.calculate(radian),0.000001);
				}
				
				@Test
				public void radNegativeBoundryTestWithDoublePI() {
					IAngleMeasurement radian = new RadianMeasurement((-1)*Maths.PI*2);
					Assert.assertEquals(1.0,Cos_FinalVersion.calculate(radian),0.000001);
				}
		...
		class DegreeBoundryTest{
			...
			class PostiveRadianBoundryTest{
				...
				public void JustAboveTestWith90() {
					...
					Assert.assertEquals(-0.001745,Cos_FinalVersion.calculate(radian),0.000001);
				}
				
				@Test
				public void JustAboveTestWith180() {
					IAngleMeasurement radian = new DegreeMeasurement(180.1);
					Assert.assertEquals(-0.999998,Cos_FinalVersion.calculate(radian),0.000001);
				}
	...
	class OutOf360Test{
		...
		class RadianOutOf360Test{
			...
			public void TestGreaterthan2PI() {
				IAngleMeasurement radian = new RadianMeasurement(Maths.PI*7/3);
				Assert.assertEquals(0.5,Cos_FinalVersion.calculate(radian),0.000001);
			}
			
			@Test
			public void TestLessthanNegative2PI() {
				IAngleMeasurement radian = new RadianMeasurement((-1)*Maths.PI*7/3);
				Assert.assertEquals(0.5,Cos_FinalVersion.calculate(radian),0.000001);
			}
		...
		class DegreeOutOf360Test{
			...
			public void TestGreaterthan360() {
				IAngleMeasurement radian = new DegreeMeasurement(420);
				Assert.assertEquals(0.5,Cos_FinalVersion.calculate(radian),0.000001);
			}
	...
	class WholeEquivalencePartitionTest{
		...
		public void TestWith45() {
			...
			Assert.assertEquals(0.707107,Cos_FinalVersion.calculate(radian),0.000001);
		}
